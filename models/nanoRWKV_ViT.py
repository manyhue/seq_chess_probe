    ...
    def __init__(self, config, layer_id):
        ...
        with torch.no_grad():
            ...
            for h in range(self.n_head):
                decay_speed[h] = -6 + 5 * (h / (self.n_head - 1)) ** (
...
class RWKV_ViT(ClassifierModule):
    ...
    def forward(self, idx):
        ...
        assert (
            t <= self.c.seq_len
        ), f"Cannot forward sequence of length {t}, block size is only {self.c.seq_len}"
    ...
    def crop_seq_len(self, seq_len):
        ...
        assert seq_len <= self.c.seq_len
        self.c.seq_len = seq_len
        self.transformer.wpe.weight = nn.Parameter(
            self.transformer.wpe.weight[:seq_len]
        )
        for block in self.transformer.h:
    ...
    def from_pretrained(cls, model_type, override_args=None):
        ...
        print("forcing vocab_size=50257, seq_len=1024, bias=True")
    ...
    def configure_optimizers(self, weight_decay, learning_rate, betas, device_type):
        ...
        decay_params = [
            p for n, p in param_dict.items() if p.dim() >= 2 and "time_" not in n
        ]
        nodecay_params = [
    ...
    def estimate_mfu(self, fwdbwd_per_iter, dt):
        ...
        cfg = self.c
        L, H, Q, T = cfg.n_layer, cfg.n_head, cfg.n_embd // cfg.n_head, cfg.seq_len
